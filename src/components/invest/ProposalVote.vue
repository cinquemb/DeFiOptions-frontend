  ...
  <div>
    ...
    <div class="d-flex flex-row" v-if="!getSellType">
      <div class="p-2" v-if="(!getCoveredType) && (!isBuyWithExchangeBalance)">
        <button @click="approveStablecoinCollateralDeposit" class="btn btn-success form-control" :disabled="(isOptionSizeNotValid.status) || (writingStepTx > 0)">
      ...
      <div class="p-2" v-if="getCoveredType">
        <button @click="approveAllowanceCovered" class="btn btn-success form-control" :disabled="(isOptionSizeNotValid.status) || (writingStepTx > 0) || getMaxOptionSize === 0">
          <span v-if="loading" class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
      ...
      <div class="p-2"  v-if="(!getCoveredType) && (!isBuyWithExchangeBalance)">
        <button @click="depositCollateral" class="btn btn-success form-control" :disabled="(isOptionSizeNotValid.status) || (writingStepTx < 1)">
          <span v-if="loading" class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
          Deposit {{buyWith}}
        </button>
      </div>
